...
describe Shop do
  describe '#update_quality' do
    let(:conjured) { double(Conjured, name: 'Conjured', sell_in: 30, quality: 30) }
    let(:brie) { double(AgedBrie, name: 'Aged Brie', sell_in: 20, quality: 30) }
    let(:backstage_pass) { double(BackstagePass, name: 'Backstage pass', sell_in: 14, quality: 43) }
    let(:sulfuras) { double(Sulfuras, name: 'Sulfuras', sell_in: 5, quality: 35) }
